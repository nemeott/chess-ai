...
def test_position(fen, move_uci=None):
    ...
    if move_uci:
        ...
        incremental_score = Score(
            ...
            init_score.eg,
            init_score.npm,
            init_score.pawn_struct,
            init_score.king_safety,
        )

        # Make the move and update the score
        move = rc.Move.from_uci(move_uci)
        ...
        # Assert scores match
        assert incremental_score.material == fresh_score.material, (
            f"Material scores do not match: {incremental_score.material} != {fresh_score.material}"
        )
        assert incremental_score.mg == fresh_score.mg, (
            f"Midgame scores do not match: {incremental_score.mg} != {fresh_score.mg}"
        )
        assert incremental_score.eg == fresh_score.eg, (
            f"Endgame scores do not match: {incremental_score.eg} != {fresh_score.eg}"
        )
        assert incremental_score.npm == fresh_score.npm, (
            f"Non-pawn material scores do not match: {incremental_score.npm} != {fresh_score.npm}"
        )
        assert incremental_score.pawn_struct == fresh_score.pawn_struct, (
            f"Pawn structure scores do not match: {incremental_score.pawn_struct} != {fresh_score.pawn_struct}"
        )
        assert incremental_score.king_safety == fresh_score.king_safety, (
            f"King safety scores do not match: {incremental_score.king_safety} != {fresh_score.king_safety}"
        )
...
if __name__ == "__main__":
    ...
    n = 1_000_000
    for i in range(n):
        new_score = score.updated(board, rc.Move.from_uci("b5c7"))
