...
from score_rc import Score  # For position evaluation
from tt_entry_rc import EXACT, LOWERBOUND, UPPERBOUND, TTEntry
...
class ChessBot:
    ...
    def __init__(self, use_opening_book: bool = True) -> None:  # TODO: Add verbose/print option
        ...
        tt_entry_size = TTEntry(np.int8(0), np.int16(0), EXACT, rc.Move("e2e4")).__sizeof__()
        ...
        self.killer_moves: list[list[rc.Move]] = [[], []]  # Killer moves for alpha-beta pruning
    ...
    def increment_moves_and_render_debug_arrow(self, depth: np.int8, move: rc.Move) -> None:
    ...
    # def quiescence(self, board: rc.Board, depth, alpha, beta, score) -> np.int16:
    #     """
    ...
    #     bitboard: rc.Bitboard = board.occupied_co[maximizing_player] # Get bitboard for current player
    ...
    #             _push(rc.Move.null()) # Make null move (skip turn)
    ...
    # def best_node_search(self, board: rc.Board, alpha, beta, maximizing_player: bool):
    #     """
    ...
    def print_stats(self, board: rc.Board, time_taken: float) -> None:
        ...
        tt_entry_size = TTEntry(np.int8(0), np.int16(0), EXACT, rc.Move.from_uci("e2e4")).__sizeof__()
