...
class ChessBot:
    ...
    def alpha_beta(self, board: chess.Board, depth: np.int8, alpha: np.int16, beta: np.int16, maximizing_player: bool, score: Score, allow_null_move: bool = True) -> tuple[np.int16, Optional[chess.Move]]:
        """
    ...
    def mtd_safe_fix(self, board: chess.Board, first_guess: np.int16, depth: np.int8, color_multiplier: np.int8, key) -> tuple[np.int16, Optional[chess.Move]]:
        """
    ...
    def mtd_fix(self, board: chess.Board, first_guess: np.int16) -> tuple[np.int16, Optional[chess.Move]]:
        ...
        while lower_bound < upper_bound:
            ...
            guess, best_move = self.alpha_beta(board, DEPTH, beta - 1, beta,
                                               board.turn, self.score, allow_null_move=False)
            if guess < beta:
                upper_bound = guess
            else:
                lower_bound = guess
    ...
    def mtd_f(self, board: chess.Board, first_guess: np.int16) -> tuple[np.int16, Optional[chess.Move]]:
        ...
        while lower_bound < upper_bound:
            ...
            guess, best_move = self.alpha_beta(board, DEPTH, beta - 1, beta, board.turn, self.score)
            if guess < beta:
                upper_bound = guess
            else:
                lower_bound = guess
